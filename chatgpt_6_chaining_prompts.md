...
Chaining of prompts helps breakdown and complex task into multiple simpler tasks.

- More Focused (breaks down a complex task)

Simpler analogy is simple sphagetti code vs modular programming

<img src="https://cdn.mathpix.com/snip/images/qTRHfPJXY1o7KgtoLtn5ASFYI-pNJ4NNLZvKdME7g_0.original.fullsize.png" />

## Chaining Prompts
* Simplifes the token
* Some prompts can be skipped in the workflow if not needed for the task
* Easier to test (human in the loop)
* For complex tasks maintain state outside the LLM (in your code)
* Allows use for external tools (web search or database
...
Define the completions method `get_completion_from_messages()`
...
Define a prompt with a system message to find the category or a product from the list of products.
...
Then we pass the actual message
...
Construct the messages array with roles of system and user and associated messages.
...
On executing the query you will get the following output
...
```
Let us try another example with the message `my router isn't working`
...
Since there are no routers, output will be an empty list

```
[]
```
Let us look at detailed product information based on Products and Categories
...
```
Define methods to get product from name and get products by category
...
On asking about "TechPro Ultrabook", you will get following output
